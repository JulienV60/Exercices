...
describe("#03_includes_array", () => {
  ...
  test("isTheComputedOneInOurNumbers value should change when you change theComputedOne", () => {
    return studentCode.then((code) => {
      const changedCode = code.replace("const theComputedOne = 42;", "const theComputedOne = 420;");
  ...
  test("It should log the right message when theComputedOne is in scientistsNumbers", () => {
    return studentCode.then((code) => {
      ...
      expect(fakeLog).toHaveBeenCalledWith(
        "The Answer to the Ultimate Question of Life, the Universe, and Everything is 42, I knew it!",
      );
      fakeLog.mockRestore();
    });
  });
  ...
  test("It should log the right message when theComputedOne is in scientistsNumbers, no matter what number you search", () => {
    return studentCode.then((code) => {
      const changedCode = code.replace("const theComputedOne = 42;", "const theComputedOne = 666;");
      ...
      expect(fakeLog).toHaveBeenCalledWith(
        "The Answer to the Ultimate Question of Life, the Universe, and Everything is 666, I knew it!",
      );
      fakeLog.mockRestore();
    });
  });

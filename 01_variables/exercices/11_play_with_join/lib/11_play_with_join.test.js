...
describe("#11_play_with_join", () => {
  test("litteralDigits should be equal to an array with numbers spelled out as strings from zero to nine", () => {
    return studentCode.then((code) => {
      ...
      expect(litteralDigits).toEqual(["zero", "one", "two", "three", "four", "five", "six", "seven", "eight", "nine"]);
    });
  });
  ...
  test("Should be equal to a string 'zero - one - two ...'", () => {
    return studentCode.then((code) => {
      ...
      expect(allDigits).toEqual("zero - one - two - three - four - five - six - seven - eight - nine");
    });
  });

  test("Should be linked to litteralDigits. Changing it should change allDigits", () => {
    return studentCode.then((code) => {
      const changedStudentCode = code.replace(new RegExp(/(let|const) allDigits/m), "litteralDigits[5] = 'cinq'; $&");
      const allDigits = eval(changedStudentCode + "; allDigits;");
      expect(allDigits).toBe("zero - one - two - three - four - cinq - six - seven - eight - nine");
    });
  });
});
